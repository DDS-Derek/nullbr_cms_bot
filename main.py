...
ALLOWED_USER_IDS = [
    int(id.strip()) for id in os.getenv("TG_CHAT_ID", "").split(",") if id.strip()
]
MP_BASE_URL = os.getenv("MP_BASE_URL")

# 初始化CMS客户端
from mp import MPClient

cms_client = MPClient(base_url=MP_BASE_URL) if MP_BASE_URL else None

# 初始化NullbrSDK
nullbr_client = NullbrSDK(
    app_id=NULLBR_APP_ID, api_key=NULLBR_API_KEY, base_url=NULLBR_BASE_URL
)
...
async def search(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # 发送加载消息
    loading_message = await update.message.reply_markdown_v2(
        escape_markdown("🔍 正在搜索，请稍候...", version=2)
    )
    ...
    try:
        ...
        if parts[0] == "detail":
            ...
            loading_message = await context.bot.send_message(
                chat_id=update.effective_chat.id, text="⏳ 正在获取详情信息..."
            )
            ...
            if media.has_115:
                keyboard.append(
                    [
                        InlineKeyboardButton(
                            "🔍 获取115资源", callback_data=f"115_{media_type}_{tmdbid}"
                        )
                    ]
                )
            if media.has_magnet:
        ...
        # 发送加载消息
        loading_message = await context.bot.send_message(
            chat_id=update.effective_chat.id, text="⏳ 正在获取资源信息，请稍候..."
        )
        keyboard = []
        if resource_type == "115":
            ...
            for i, item in enumerate(resources.items):
                ...
                message += (
                    f"🔗 链接：`{escape_markdown(item.share_link, version=2)}`\n\n"
                )
            ...
            if media_type == "movie":
                ...
                for i, item in enumerate(resources.magnet):
                    ...
                    message += (
                        f"🔗 链接：`{escape_markdown(item.magnet, version=2)}`\n\n"
                    )
                    if cms_client and is_user_allowed(update.effective_user.id):
                        keyboard.append(
                            [
...
async def season_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        for i, item in enumerate(resources.magnet):
            ...
            message += f"💾 大小：{escape_markdown(item.size, version=2)}\n"
            message += (
                f"🎬 分辨率：{escape_markdown(item.resolution or '未知', version=2)}\n"
            )
            message += f"📺 来源：{escape_markdown(item.source or '未知', version=2)}\n"
            message += (
                f"⚡️ 质量：{escape_markdown(item.quality or '未知', version=2)}\n"
            )
            message += f"🈶 中字：{'是' if item.zh_sub else '否'}\n"  # 布尔值不需要转义
            ...
            if cms_client and is_user_allowed(update.effective_user.id):
                keyboard.append(
                    [
                        InlineKeyboardButton(
                            f"📥 转存 #{i + 1}", callback_data=f"cms_{item.magnet}"
                        )
                    ]
                )
        # 发送资源信息
...
async def cms_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        # 发送加载消息
        loading_message = await context.bot.send_message(
            chat_id=update.effective_chat.id, text="⏳ 正在转存资源，请稍候..."
        )
        ...
        if result.get("code") == 200:
            ...
            await loading_message.edit_text(
                f"❌ 转存失败：{result.get('msg', '未知错误')}"
            )

    except Exception as e:
        logging.error(f"CMS callback error: {str(e)}")
        await context.bot.send_message(
            chat_id=update.effective_chat.id, text="转存资源时发生错误，请稍后重试。"
        )
